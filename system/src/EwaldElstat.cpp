/*
 * A class to compute electrostatic interactions using basic Ewald summation
 * Author JJ, Date Aug 2022
 *
 * Implementation of CalculateForces was inspired by MACSIMUS code (the order of sums...)
 */
...
EwaldElstat::EwaldElstat(SimulatedSystem *parent, double cut, double alp, double kap, unsigned int shift)
...
// copy constructor
EwaldElstat::EwaldElstat(const EwaldElstat &system)
...
double EwaldElstat::CalculateForces()
{
    double Eel = 0.0;
    double Erinter = 0.0;
    double Erintra = 0.0;
...
#pragma omp parallel num_threads(thread_count) reduction(+ \
                                                         : Erinter)
    {
        ...
        while ((r2 = parent->pairlist->GetNextPair(pair, cutoff2)) > 0.0)
        {
            ...
            alphar = alpha * rnorm;
            erfcrr = erfc(alphar) / rnorm;
            Erinter += q2 * (erfcrr - shiftE);
            force = q2 / r2 * (M_2_SQRTPI * alpha * exp(-alphar * alphar) + erfcrr) - q2 * shiftf;
...
double EwaldElstat::CalculateEpot() const
{
    double Eel = 0.0;
    double Erinter = 0.0;
    double Erintra = 0.0;
...
double EwaldElstat::CalculateCutoffCorrection()
{
    Ecutoffcorr = 0.0;
    return 0.0;
}
...
int EwaldElstat::SetCutoff(double vdwcut, double cut, int boundaryC)
{
    if (boundaryC == 0)
    {
...
// cloning
EwaldElstat *EwaldElstat::copy(SimulatedSystem *newparent) const
{
    EwaldElstat *newEwaldElstat = new EwaldElstat(*this);
    newEwaldElstat->SetParent(newparent);
    return newEwaldElstat;
}

// intramol electrostatic interactions
// initialization of intramol bonds
int EwaldElstat::InitializeIntramolBonds(double LJ14factor, double el14factor) const
{
    int noInter = 0;
    int i, j, k;
    Molecule *mol;
...
void EwaldElstat::PrintInfo(std::ofstream &stream, double u_eng, std::string engunit) const
{
