#ifndef EWALDELSTATHEADER
#define EWALDELSTATHEADER
 ...
 * @brief Standard (slow) Ewald summation for charged systems. Includes both the \f$r\f$-space and \f$k\f$-space sums.
 *
 ...
 *
 * The Ewald summation for electrostatic interactions is invoked by the following directive in `.control` file:
 * ```(control)
 * elstat ewald cutoff [alpha kappa]
 * ```
 ...
 */
class EwaldElstat : public AbstractInterMolField
{
private:
  /**
   ...
   */
  EwaldElstat(); // making default constructor private disables its use
  ///@{
  /**
...
public:
   ...
   */
  EwaldElstat(SimulatedSystem *parent, double cut, double alp = -1.0, double kap = -1.0, unsigned int shift = 3); // constructor
  /**
   * @brief EwaldElstat copy constructor
   *
   * @param system EwaldElstat class to be copied.
   */
  EwaldElstat(const EwaldElstat &system); // copy constructor
  /**
   * @brief EwaldElstat destructor
   *
   * The destructor is responsible for deallocation of arrays @ref kx, @ref ky, @ref kz, @ref expk, @ref real and @ref imag.
   */
  ~EwaldElstat(); // destructor
  /**
   * @brief Calculate forces caused by electrostatic interactions (intermolecular).
   * Calculate forces caused by this field (EwaldElstat) and add them to @ref Atom::force.
   ...
   *
   * @par See also
   * @ref EwaldElstat(const EwaldElstat&), @ref SetParent(SimulatedSystem*), @ref AbstractInterMolField::copy(SimulatedSystem *) const, @ref AbstractIntraMolField::copy(Molecule *) const.
   *
   * @param newparent Parent SimulatedSystem of the newly created EwaldElstat object.
   * @return (Pointer to) the newly created EwaldElstat object.
   */
  EwaldElstat *copy(SimulatedSystem *newparent) const override; // cloning itself
  /**
