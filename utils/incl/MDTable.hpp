// print table to .md
// original source: https://github.com/friedmud/variadic_table
// modified to print in markdown and to correctly calculate width of floats
#pragma once
 ...
 */
enum class MDTableColumnFormat
{
    AUTO,
    SCIENTIFIC,
    FIXED,
...
/**
 * A class for "pretty printing" a table of data. Originally by Derek Gaston (see https://github.com/friedmud/variadic_table).
 * The original version was modified to print in MarkDown format and to correctly calculate the width of floating point numbers.
 *
...
public:
     ...
     * @param cell_padding The padding between the content and borders of the cell.
     */
    MDTable(std::vector<std::string> headers,
     ...
     *
     * @param entries A Tuple of data to add
     */
    ...
    void print(StreamType &stream)
    {
        ...
        // stream << std::string(total_width, '-') << "\n"; // original must be modified
        auto &headerrow = _data[0];
        stream << "|";
        print_each_header(headerrow, stream);
        stream << "\n";
        ...
        // Print out the newline below the table
        stream << "\n";
    }
     ...
     *
     * @param column_format The format for each column: MUST be the same length as the number of columns.
     */
    void setColumnFormat(const std::vector<MDTableColumnFormat> &column_format)
    {
     ...
     * Note: this is ignored for std::string columns
     *
     * @param precision The precision for each column: MUST be the same length as the number of columns.
     */
...
protected:
    ...
    /**
     * This gets called on each header
     */
    ...
    void print_each(TupleType &&t, StreamType &stream, std::integral_constant<size_t, I>)
    {
        ...
        if (!_column_format.empty())
        {
            ...
            if (_column_format[I] == MDTableColumnFormat::SCIENTIFIC)
                stream << std::scientific;

            if (_column_format[I] == MDTableColumnFormat::FIXED)
                stream << std::fixed;

            if (_column_format[I] == MDTableColumnFormat::PERCENT)
    ...
    // now the same for headers (JJ)
    /**
     *  This ends the recursion
     */
    template <typename TupleType, typename StreamType>
    void print_each_header(TupleType &&,
                           StreamType & /*stream*/,
                           std::integral_constant<
                               size_t,
                               std::tuple_size<typename std::remove_reference<TupleType>::type>::value>)
    ...
    template <std::size_t I,
              ...
              typename = typename std::enable_if<
                  I != std::tuple_size<typename std::remove_reference<TupleType>::type>::value>::type>
    void print_each_header(TupleType &&t, StreamType &stream, std::integral_constant<size_t, I>)
    {
        auto &val = std::get<I>(t);

        if (justify<decltype(val)>(0) == std::right)
        {
            stream << std::string(_cell_padding, ' ') << std::string(_column_sizes[I] - 1, '-') << ":" << std::string(_cell_padding, ' ') << "|";
        }
        else
        {
            stream << std::string(_cell_padding, ' ') << ":" << std::string(_column_sizes[I] - 1, '-') << std::string(_cell_padding, ' ') << "|";
        ...
        // Recursive call to print the next item
        print_each_header(std::forward<TupleType>(t), stream, std::integral_constant<size_t, I + 1>());
    }
     ...
     */
    template <typename TupleType, typename StreamType>
    void print_each_header(TupleType &&t, StreamType &stream)
    {
        print_each_header(std::forward<TupleType>(t), stream, std::integral_constant<size_t, 0>());
    }
     ...
     * Try to find the size the column will take up
     *
     * If the datatype is a float - let's try to find its length (JJ)
     */
    template <class T>
    size_t sizeOfData(const T &data,
    ...
    void size_columns()
    {
        ...
        if (!_precision.empty())
        {
            ...
            {
                if (_precision[i] > 6)
                    _column_sizes[i] += _precision[i] - 6;
            }
        }
    }
    ...
    /// Column Format
    std::vector<MDTableColumnFormat> _column_format;
