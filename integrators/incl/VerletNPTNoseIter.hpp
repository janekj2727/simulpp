#ifndef VERLETNPTNOSEITERHEADER
#define VERLETNPTNOSEITERHEADER

#include "AbstractVerletIntegrator.hpp"

class VerletNPTNoseIter : public AbstractVerletIntegrator
{
private:
    double tau_T;
    ...
    double recM_P;      // 1/M_P
    int noiter;
    int VerletStep(Molecule *mol, double hvXiPredold, double hvLamPredold, bool isFirst);
    ...
    int SimplePredict(std::vector<Molecule>::iterator itmol);
    int StoreVelocity(std::vector<Molecule>::iterator itmol);
    int VelocityToPred(std::vector<Molecule>::iterator itmol);
...
public:
    ...
    VerletNPTNoseIter(double step, SimulatedSystem *simsystem, Vector trvpB, double Tfinal, double tauT, double Pfinal, double tauP, bool startWithShake, int noIter); // with TRVP
    VerletNPTNoseIter(double step, SimulatedSystem *simsystem, double Tfinal, double tauT, double Pfinal, double tauP, bool startWithShake, int noIter);               // without TRVP
    ~VerletNPTNoseIter();
};
