 ...
 * Alejandro – Trotter decomposition scheme, without TRVP (with TRVP the algorithm is completely different – defined in VerletNPTNoseTRVP.cpp)
 * for trial simulations
 * Author JJ, Date Oct 2022
 */
...
/*
 *  Nosé–Hoover thermostat implemented according to the Tuckerman+Alejandro 2006 article
 *  Compare with VerletNPTNose.cpp
 */
...
/**
 * @brief Calculate \f$\exp(\alpha)\frac{\sinh(\alpha)}{\alpha}\f$ (by Taylor expansion)
 *
 * @param alpha Parameter expected to be close to zero.
 * @return double \f$\exp(\alpha)\frac{\sinh(\alpha)}{\alpha}\f$.
 */
inline double ExpSinh(double alpha)
{
    return exp(alpha) * (1.0 + pow(alpha, 2.0) / 6.0 + pow(alpha, 4.0) / 120.0 + pow(alpha, 6.0) / 5040.0 + pow(alpha, 8.0) / 326880.0);
}

// initialization of this integrator (main part is done in AbstractVerletIntegrator construction)
VerletNPTAlejandre::VerletNPTAlejandre(double step, SimulatedSystem *simsystem, double Tfinal, double tauT, double Pfinal, double tauP, bool startWithShake)
...
// empty destructor
VerletNPTAlejandre::~VerletNPTAlejandre()
{
    delete Xi;
    delete Lambda;
    delete Xi_old;
...
int VerletNPTAlejandre::Integrate(int Nsteps)
{
    int i, l;
    Molecule *p_mol;
    ...
    for (l = 0; l < Nsteps; l++)
    {
        ...
        // thermostat h/2
        ThermostatHalfStep(Ekininst, velscale);
        ...
        // barostat h/2
        BarostatHalfStep2(Ekininst);

#ifdef PARALLEL
#pragma omp parallel for num_threads(thread_count) private(i, p_mol)
#endif
        for (i = 0; i < system->noMolecules; i++)
        {
            p_mol = &(system->molecules[i]);

            // Verlet velocity update (velocity update with old forces)
            VerletStep2(p_mol);
        }

        // Barostat full-step (update of box, atom-based)
        BarostatFullStep1();
...
#pragma omp parallel for num_threads(thread_count) private(i, p_mol)
        ...
        for (i = 0; i < system->noMolecules; i++)
        {
            ...
            // SHAKE
            Shake(p_mol, virialc); // virialc protected inside by omp atomic
        }
...
#pragma omp parallel for num_threads(thread_count) private(i, p_mol)
        ...
        for (i = 0; i < system->noMolecules; i++)
        {
            ...
            Rattle(p_mol, virialc); // virialc protected inside by omp
        }
...
double VerletNPTAlejandre::EnergyOfExtraDOF() const
{
...
int VerletNPTAlejandre::ThermostatHalfStep(double &Ekin, double &velscale)
{
    double scaling;
    static double Ekinfinaltotal = 0.5 * (system->noDegrOfFred + 1) * T_f; // including Ekin of barostat

    // integration of xi here (by h/8)
    Xi->operator()(0) += Xi->operator()(1) / 8.0;

    // xi'(t+h/4) = xi'(t) + h/4*(2Ekininst - 2Ekinfinal + M_P*v_lambda^2 - kB*T_final)/M_T
    ...
    Xi->operator()(1) += h * h / 4.0 * (2 * Ekin + M_P * Lambda->operator()(1) * Lambda->operator()(1) / h / h - 2.0 * Ekinfinaltotal) / M_T;

    // v(t) = v(t) * exp(-xi'(t+h/4)*h/2)
    //     (E_kininst(new) = 0.5 * sum(m*(v*exp(-xi'(t+h/4)*h/2))^2)
    //      E_kininst(new) = E_kininst * exp(-2*xi'(t+h/4)*h/2))
    scaling = exp(-Xi->operator()(1) / 2.0);
    // system->RescaleVelocities(scaling); // velocities can be rescaled later here only save scaling
    velscale *= scaling;
    // lambda'(t) scaling
    Lambda->operator()(1) *= scaling;
    // kinetic energy rescaled immediately
    Ekin *= scaling * scaling;

    // integration of xi (by h/4) in analogy with NVT Nose...
    Xi->operator()(0) += Xi->operator()(1) / 4.0;

    // xi'(t+h/2) = xi'(t+h/4) + h/4*(2Ekininst - 2Ekinfinal + M_P*v_lambda^2 - kB*T_final)/M_T
    // M_P*v_lambda^2 ... 2*kinetic energy connected with lambda
    // kB*T_f ... 2*desired kinetic energy of lambda
    // Ekinfinal + kB*T_f = Ekinfinaltotal
    Xi->operator()(1) += h * h / 4.0 * (2 * Ekin + M_P * Lambda->operator()(1) * Lambda->operator()(1) / h / h - 2.0 * Ekinfinaltotal) / M_T;
...
// half step of barostat (lambda' integration)
int VerletNPTAlejandre::BarostatHalfStep2(double Ekin)
{
    ...
    // lambda'(t+h/2) = lambda'(t) + 3*h/2*((P(t) - Pfinal)*V/M_P + 2Ekin/N_f/M_P)
    // P(t) = Pkin + Pvir + Pcorr
    Lambda->operator()(1) += 1.5 * h * h * ((Pkin + Pvir - P_f) * V + 2.0 * Ekin / system->noDegrOfFred) / M_P;

    return 0;
}

// full step of barostat, box size integration
int VerletNPTAlejandre::BarostatFullStep1()
{
    // lambda(t+h) = lambda(t) + h * lambda(t+h/2)
    Lambda->operator()(0) += Lambda->operator()(1);
    return 0;
}

// VV1 – 1st part of velocity verlet
int VerletNPTAlejandre::VerletStep(Molecule *p_mol)
{
    int j, k;
    Matrix *atomR;
    double alpha = Lambda->operator()(1) * 0.5; // lambda'(t+h/2)*h/2
    double scaling = ExpSinh(alpha);
    ...
    for (j = 0; j < p_mol->noAtoms; j++)
    {
        ...
        for (k = 0; k < 3; k++)
        {
            // r(t+h) = r(t) + h * v(t+h/2) * exp(alpha)
            // scaling done before with the entire box
            ATOMR(0, k) += ATOMR(1, k) * scaling;
        }
    }
...
int VerletNPTAlejandre::VerletStep2(Molecule *p_mol)
{
    int j, k;
    Matrix *atomR;
    double alpha = (1.0 + 3.0 / system->noDegrOfFred) * Lambda->operator()(1) * 0.25;
    double exp2alpha = exp(-alpha * 2.0);
    double scaling = ExpSinh(-alpha);
    ...
    for (j = 0; j < p_mol->noAtoms; j++)
    {
        ...
        for (k = 0; k < 3; k++)
        {
            // v(t+h/2) = v(t)*exp(-2alpha) + h/2 * f(t+h)/m * exp(-alpha) * sinh(alpha)/alpha
            // ATOMR(1, k) += h * h * 0.5 * p_mol->atoms[j].force[k]/p_mol->atoms[j].mass;  // original NVE version
            ATOMR(1, k) = ATOMR(1, k) * exp2alpha + h * h * 0.5 * p_mol->atoms[j].force[k] / p_mol->atoms[j].mass * scaling;
            // ATOMR(1, k) = ATOMR(0, k) - ATOMR(2, k) + h * h * 0.5 * p_mol->atoms[j].force[k] / p_mol->atoms[j].mass; // MTTK with SHAKE
        }
    }
...
int VerletNPTAlejandre::TimeShift()
{
    Matrix *atomR;
    int i, j, k;
...
double VerletNPTAlejandre::Shake(Molecule *mol, double &virial)
{
...
#ifdef SCALINGVELINSHAKE
    double alpha = (1.0 + 3.0 / system->noDegrOfFred) * Lambda->operator()(1) * 0.25;
    double scaling = ExpSinh(-alpha); // maybe this scaling from VerletStep2 should be correct???
#else
    const double scaling = 1.0; // can be replaced by the two lines above, which is correct???
                                // seems to have no or only a little effect
#endif
    ...
    for (m = 0; m < iterShake; m++)
    {
        ...
        for (i = 0; i < mol->noConstrBonds; i++)
        {
            ...
            for (k = 0; k < 3; k++)
            {
                ...
                ATOMR(1, k) += mol->atoms[j].constraintForces[k] * scaling;
...
double VerletNPTAlejandre::Rattle(Molecule *p_mol, double &virial)
{
    double maxRelErr = 0.0;
...
int VerletNPTAlejandre::InitLambda()
{
    double Ekininst;
    std::vector<Molecule>::iterator itmol;
...
int VerletNPTAlejandre::PrepareConfig()
{
    int i, j, k;
    int configLevel;
    double Ekin = 0.0;
...
// new initialize
int VerletNPTAlejandre::Initialize(bool startWithShake)
{
    int i, j, k;
    Matrix *atomR;
