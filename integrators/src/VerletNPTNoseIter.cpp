/*
 * An implementation of Verlet integration method (Verlet + Nosé–Hoover thermostat + barostat + iterations to SCF)
 ...
 * and DL_POLY implementation of iterative scheme.
 * Inspired by the Nose–Hoover thermostat implementation in VerletNVTNoseIter.cpp
 */
...
VerletNPTNoseIter::VerletNPTNoseIter(double step, SimulatedSystem *simsystem, double Tfinal, double tauT, double Pfinal, double tauP, bool startWithShake, int noIter)
...
VerletNPTNoseIter::VerletNPTNoseIter(double step, SimulatedSystem *simsystem, Vector trvpB, double Tfinal, double tauT, double Pfinal, double tauP, bool startWithShake, int noIter)
...
// destructor
VerletNPTNoseIter::~VerletNPTNoseIter()
{
    delete Xi;
    delete Lambda;
    delete Xi_old;
...
int VerletNPTNoseIter::Integrate(int Nsteps)
{
    int l, j;
    double hvLamP = 0.0;       // predicted velocity of Lambda
    double hvXiPold = 0.0;     // old predicted value to next iteration
    double hvLambdaPold = 0.0; // old predicted value to next iteration
    ...
    std::vector<Molecule>::iterator itmol;
    bool isfirst = true; // true if first iteration
    ...
    for (l = 0; l < Nsteps; l++)
    {
        ...
        // lambdaP(t+h) = lambda(t) + h lambda'(t+h/2)^P = lambda(t) + h (lambda(t)'^P - 0.5 lambda(t-h/2)')
        // boxLengthP(t+h)/boxLength(t) = exp(h (lambda'(t)^P - 0.5 * lambda'(t-h/2)))
        boxscalePred = exp(Lambda->operator()(predVel) - 0.5 * Lambda->operator()(2));
...
#ifdef PARALLEL
#pragma omp parallel for num_threads(thread_count) private(itmol) reduction(+ \
                                                                            : Ekininst)
#endif
            for (itmol = system->molecules.begin(); itmol != system->molecules.end(); itmol++)
            {
                // one step of Verlet (new position calculation), uses predicted value of h*xi' and h*lambda'
                VerletStep(&(*itmol), hvXiPold, hvLambdaPold, isfirst);

                // SHAKE (shakeType = -3)
                Shake(&(*itmol), system->virconstr, boxscalePred); // system->virconstr protected by omp atomic inside
            ...
            // store old velocity
            hvLambdaPold = Lambda->operator()(predVel);
            // lambda velocity
            VelocityVec(Lambda);
            // update predicted velocity
            Lambda->operator()(predVel) = Lambda->operator()(1);

            // update predicted scaling
            // boxLength(t+h)/boxLength(t) = exp(Lambda(t+h) - Lambda(t))
            boxscalePred = exp(Lambda->operator()(3) - Lambda->operator()(0));
...
int VerletNPTNoseIter::VerletStep(Molecule *mol, double hvXiPredold, double hvLamPredold, bool isFirst)
{
    int j, k;
    Matrix *atomR;
    double hvXiPred = Xi->operator()(predVel);
    double hvLamPred = Lambda->operator()(predVel);
    ...
    else // iterations, only update
    {
        for (j = 0; j < mol->noAtoms; j++)
        {
            ...
            for (k = 0; k < 3; k++)
            {
                ATOMR(3, k) -= ATOMR(predVel, k) * (hvXiPred + (1.0 + 3.0 / system->noDegrOfFred) * hvLamPred) - mol->atoms[j].errorG->operator()(k) * (hvXiPredold + (1.0 + 3.0 / system->noDegrOfFred) * hvLamPredold);
...
double VerletNPTNoseIter::EnergyOfExtraDOF() const
{
...
// Initialize Xi
int VerletNPTNoseIter::InitXi(int predVel, double Ekininst)
{
    double force;
    int m;
    ...
    if ((system->extendedDOFs != nullptr) && (system->extendedDOFs->GetNumberOfRows() > 2))
    {
        ...
        {
            print_warning(0, "Extended DOFs from .config don't start by Xi, cannot use info from .config file\n");
            fromconfig = false;
        }
    }
...
int VerletNPTNoseIter::InitLambda(int predVel, double Ekininst)
{
    ...
    if (fromconfig)
    {
        ...
        else
        {
            ...
            if (fabs(Lambda->operator()(2)) > h * h * 100) // maybe pressure was stored on this position by MTTK/Alejandre version of MTK NPT
            {
                ...
                "Check if you start from the configuration (.config) created by VerletNPTNoseIter integrator.\n",
...
int VerletNPTNoseIter::TimeShiftVec(Vector *vec)
{
    int i;
...
double VerletNPTNoseIter::TRVPvec(Vector *vec)
{
    int i;
    if (predVel != 1)
    {
    ...
    { // simple predict
        vec->operator()(1) = 2.0 * vec->operator()(2) - vec->operator()(1);
    }
    return vec->operator()(predVel);
}

// velocity of the extended degree of freedom
double VerletNPTNoseIter::VelocityVec(Vector *vec)
{
    double vhm, vhp;
    ...
    vec->operator()(1) = sqrt(0.5 * (vhm * vhm + vhp * vhp)) * ((vhp > 0) - (vhp < 0));
#endif
    return vec->operator()(1);
}

// Calculate velocity according to VERLET #define and return kinetic energy
double VerletNPTNoseIter::VelocityCalculation(Molecule *mol)
{
...
#else               // VERLET == 3  energy - averages of both shifted values v(t) = [r(t + h) − r(t)]/h and v(t) = [r(t) − r(t − h)]/h.
            ATOMR(1, k) = sqrt(0.5 * (vhm * vhm + vhp * vhp)) * ((vhp > 0) - (vhp < 0));
#endif
...
int VerletNPTNoseIter::TimeShift()
{
    Matrix *atomR;
    int i, j, k, l;
...
int VerletNPTNoseIter::PrepareConfig()
{
    int configLevel;
    double temp = 0.0;
    std::vector<Molecule>::iterator itmol;
...
int VerletNPTNoseIter::RestoreExtendedDOFs()
{
    (*Lambda) = (*Lambda_old);
    (*Xi) = (*Xi_old);
    return 0;
...
int VerletNPTNoseIter::SimplePredict(std::vector<Molecule>::iterator itmol)
{
    int j, k;
    Matrix *atomR;
    for (j = 0; j < itmol->noAtoms; j++)
    {
        atomR = itmol->atoms[j].R;
        for (k = 0; k < 3; k++)
        {
            // velocity as vhm + 0.5*f/m
            ATOMR(1, k) = ATOMR(2, k) + 0.5 * h * h * itmol->atoms[j].force[k] / itmol->atoms[j].mass;
        }
    }
    return 0;
}
...
int VerletNPTNoseIter::StoreVelocity(std::vector<Molecule>::iterator itmol)
{
    int j, k;
    Matrix *atomR;
    ...
    for (j = 0; j < itmol->noAtoms; j++)
    {
        ...
        {
            // zero constrerrG (used to store old velocities)
            itmol->atoms[j].errorG->operator()(k) = ATOMR(predVel, k);
        }
    }
    return 0;
}
...
int VerletNPTNoseIter::VelocityToPred(std::vector<Molecule>::iterator itmol)
{
    int j, k;
    Matrix *atomR;
